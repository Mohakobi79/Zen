...
function connect(PORT) {
  app.get("/", (req, res) =>
    ...
    <head>
        ...
        <link rel="icon" href="https://api-zenn.vercel.app/images/logo.webp" type="image/x-icon">
        <link rel="shortcut icon" href="https://api-zenn.vercel.app/images/logo.webp" type="image/x-icon">
        <title>Zenn MD Bot</title>
    ...
    <div>Bot: Online</div>
        ...
        <div class="container text-center">
            <a href="https://wa.me/62895373137660" class="btn btn-custom btn-primary" target="_blank">Use Bot</a>
        </div>
    <br>
        <div class="container text-center">
            <a href="#" class="btn btn-custom btn-primary" target="_blank">Visit My Website</a>
        </div>
  ...
  app.get("/speedtest", (req, res) => {
    exec("speedtest", (error, stdout, stderr) => {
      if (error) {
        res
          .status(500)
          .send(`<p>Speedtest failed</p><p>Error: ${error.message}</p>`);
        return;
      }
  ...
  app.get("/ping2", async (req, res) => {
    ...
    for (let i = 0; i < 10; i++) {
      try {
        ...
        pingResults.push(
          `Ping result ${i + 1}: ${response.data} ${response.status}<br />`
        );
      } catch (error) {
